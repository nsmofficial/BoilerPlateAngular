import { HttpClient } from '@angular/common/http';
import { Component, OnInit } from '@angular/core';
import { UnsubscribeOnDestroyAdapter } from 'src/app/shared/UnsubscribeOnDestroyAdapter';

@Component({
  selector: 'app-example-docket-table',
  templateUrl: './example-docket-table.component.html'
})
export class ExampleDocketTableComponent  extends UnsubscribeOnDestroyAdapter implements OnInit{
// for example purpose only, local json data is used, in production a api service should be used instead.
jsonUrl = '../../../assets/data/docketDetails.json'
data: [] | any;
tableload = true; // flag , indicates if data is still lodaing or not , used to show loading animation 
csv: any[];
addAndEditPath: string
drillDownPath: string
uploadComponent: any;
csvFileName: string; // name of the csv file, when data is downloaded , we can also use function to generate filenames, based on dateTime. 
companyCode: number;
/*Below is Link Array it will Used When We Want a DrillDown
 Table it's Jst for set A Hyper Link on same You jst add row Name Which You
 want hyper link and add Path which you want to redirect*/
linkArray = [
  { Row: 'Count', Path: 'example/drillDown' },
]
toggleArray=[
  'activeFlag',
  'isActive',
  'isActiveFlag',
  'isMultiEmployeeRole'
]
//Warning--It`s Used is not compalsary add when you want.
/*End*/
//#region create columnHeader object,as data of only those columns will be shown in table.
// < column name : Column name you want to display on table > 

columnHeader = {
  "checkBoxRequired": "",
  "DocketBranch": "Docket Branch",
  "Count": "Count",
  "TotalPackage": "Total Package",
  "TotalWeight": "Total Weight",
  "TotalCFT": "Total CFT"
}

METADATA = {
  checkBoxRequired: true,
  // selectAllorRenderedData : false,
  noColumnSort: ['checkBoxRequired']
}
dynamicControls={
  add:false,
  edit:false,
  csv:true
}
//#endregion
//#region declaring Csv File's Header as key and value Pair
headerForCsv = {
  "DocketBranch": "Docket Branch",
  "Count": "Count",
  "TotalPackage": "Total Package",
  "TotalWeight": "Total Weight",
  "TotalCFT": "Total CFT",
}
//#endregion

//declaring breadscrum
breadscrums = [
  {
    title: "Table",
    items: ["example"],
    active: "Table"
  }
]
IscheckBoxRequired: boolean;
// declararing properties

constructor(private http: HttpClient) {
  super();
  this.csvFileName = "exampleUserData.csv";
  this.addAndEditPath = 'example/form';
  this.IscheckBoxRequired = true;
  this.drillDownPath = 'example/drillDown'
  //.uploadComponent = undefined;


}

ngOnInit(): void {
  
  this.http.get(this.jsonUrl).subscribe(res => {
    this.data = res;
    this.csv = this.data['data'];
    console.log(this.csv);
    this.tableload = false;

  });
  try {
    this.companyCode = parseInt(localStorage.getItem("CompanyCode"));
  } catch (error) {
    // if companyCode is not found , we should logout immmediately.
  }

}


}
